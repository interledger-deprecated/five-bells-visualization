...
class Crawler extends emitter {
  ...
  * crawlLedger (ledgerPrefix, ledgerHost) {
    ...
    for (let person of connectors) {
      ...
      if (person.connector) {
        ...
        yield this.crawlConnector(person.connector)
      } else {
  ...
  * crawlConnector (connectorHost) {
    const res = yield request.get({uri: connectorHost + '/pairs', json: true})
